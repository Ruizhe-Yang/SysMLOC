...
function ToSpacePhasor "Conversion from polyphase input to space phasor and zero sequence component" import Modelica.Constants.pi;
...
protected parameter Integer m = size(x, 1) "Number of phases" annotation ( Evaluate = true );
...
algorithm y := TransformationMatrix*x;
y0 := 1/m*sum(x);
annotation ( Inline = true , Documentation ( info = "<html>Transformation of polyphase values (of voltages or currents) to space phasor and zero sequence value.</html>" ) );
end ToSpacePhasor;
